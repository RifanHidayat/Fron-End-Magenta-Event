...
function InOut(props) {
  ...
  const deleteData = (id) => {
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        return axios
          .delete("http://localhost:3000/api/pict/add-saldo-pictb/" + id)
  ...
  useEffect(() => {
    ...
    getAcounts().then((response) => {
      response.data.map((values) => {
        ...
        if (values.id === 108 || values.id === 100 || values.id === 101) {
        } else {
        ...
        <CCardBody>
          <Formik
            ...
            onSubmit={(values, { setSubmitting }) => {
              ...
              axios
                .post("http://localhost:3000/api/pict/add-saldo-pictb", data)
                .then((response) => {
                  getAllInOutDataCostProject();
                  detailPIC();
