...
function InOut(props){
    ...
      }, 
      ...
      },
      {
        ...
        <CButton color="secondary" size="sm" onClick={()=>deleteData(row.inout_number)} ><i class="fa fa-trash"></i></CButton>  
        </CTooltip>  
        </div>
      },
  ...
  $(document).on('input', '#amount_in_out', function(e) {
   ...
   const editData=(
    ...
    $('#amount_in_out').val(amount.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1."))
    var option_account_in={value:account_in_id,label:account_in_name+ ` (${account_in_number})`}
    var option_account_out={value:account_out_id,label:account_out_name+ ` (${account_out_number})`}
    ...
    const deleteData=(id)=>{
      Swal.fire({
        ...
        preConfirm: () => {
            ...
            return axios.delete('http://localhost:3000/api/inout/'+id)
    ...
    useEffect(()=>{
       ...
       getAcounts().then((response)=>{
        response.data.map((values)=>{
            var data={value:values.id,label:values.bank_name+` (${values.account_number})`}
    ...
    <div>
      ...
      onSubmit={(values, { setSubmitting }) => {    
          ...
          var data={
              ...
              amount:$('#amount_in_out').val().replace(/[^\w\s]/gi, ''),
        ...
        if (idInOutTransaction===''){
            axios.post("http://localhost:3000/api/inout",data).then(response=>{
                getAllInOutDataCostProject();
            ...
            axios.patch("http://localhost:3000/api/inout/"+inoutNumber,data).then(response=>{
                getAllInOutDataCostProject();
        ...
        <form onSubmit={handleSubmit} autoComplete="off">  
         <CFormGroup row>
                ...
                <CFormGroup>
                 <CLabel htmlFor="total">jumlah</CLabel>
                  <CInputGroup>
                    ...
                    <CInput required  style={{textAlign:'right'}} id="amount_in_out" name="amount_in_out" onChange={handleChange}   />               
                  </CInputGroup>
                </CFormGroup>             
                </CCol>      
              ...
              {idCostTransactions===''?
                  ...
                  <CButton type="submit"  size="sm xs-2"  className="btn-brand mr-1 mb-1" color='primary'>
